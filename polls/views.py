...
def index(request):
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            single_threshold = form.cleaned_data['single_threshold']
            process_todo=[]
            if(greyscale==True):
            ...
            if(single_threshold==True):
                process_todo.append("single_threshold")
            if process_todo:
                ...
                return HttpResponse("Please at least select one option!")
        else:
            return HttpResponse("Check image and params!")
    else:
        form = DocumentForm()
